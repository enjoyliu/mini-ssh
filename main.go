...
func main() {
	...
	for {
		...
		// config with auth
		config := getServerConfigWithAuth(err)
...
func getServerConfigWithAuth(err error) *ssh.ServerConfig {
	config := &ssh.ServerConfig{
		PasswordCallback: func(conn ssh.ConnMetadata, password []byte) (*ssh.Permissions, error) {
			// Check password here
			if conn.User() == "root" && string(password) != "1234" {
				return nil, errors.New(fmt.Sprintf("password rejected for %q", conn.User()))
			}
			return nil, nil
		},
	}
